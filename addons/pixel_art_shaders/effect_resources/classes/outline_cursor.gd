...
@tool
class_name OutlineCursorEffect
...
@export var cursor_radius: float = 50.0:
	set(val):
		if cursor_radius != val:
			cursor_radius = val
			emit_changed()
...
@export var cursor_border_radius: float = 150.0:
	set(val):
		if cursor_border_radius != val:
			cursor_border_radius = val
			emit_changed()
...
@export var cursor_inverse: bool = false:
	set(val):
		if cursor_inverse != val:
			cursor_inverse = val
			emit_changed()
